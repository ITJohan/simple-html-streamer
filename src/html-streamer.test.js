...
Deno.test(`${registerIslands.name} reads islands dir and returns script element that lazy loads island script`, async () => {
  ...
  await Deno.mkdir("./islands");
  await Deno.writeFile("./islands/island-1.js", encoder.encode("island-1"));
  await Deno.writeFile("./islands/island-2.js", encoder.encode("island-2"));
  await Deno.writeFile("./islands/island-3.js", encoder.encode("island-3"));
  const expected = `
    <script type="module">
  ...
  // Act
  const generator = await registerIslands("./islands/");
  const actual = consumeGenerator(generator);
  await Deno.remove("./islands", { recursive: true });
